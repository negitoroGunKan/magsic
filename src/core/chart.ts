...
import { BPMChange, ChartNote } from './types';
...
export interface ParsedChart {
  ...
  layoutChanges: LayoutChangeEvent[];
}
 ...
 */
export function parseChart(json: any): ParsedChart {
  ...
  // 3. Parse Notes
  const notes: ChartNote[] = (json.notes || []).map((n: any) => ({
  ...
  const layoutChanges: LayoutChangeEvent[] = [];
