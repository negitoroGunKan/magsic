...
import { applyModifiers as _applyModifiers } from './src/core/modifiers';
(() => {
    ...
    function startTitleLoop() {
        if (!logo || !startScreen || !titleBgVideo)
            return;
        if (titleAnimRequestId)
    ...
    if (btnCloseCustomResults) {
        btnCloseCustomResults.addEventListener('click', () => {
            if (customResultScreen)
                customResultScreen.style.display = 'none';
            stopResultBlinking();
            // Reset state
            isTrackFailed = false;
            shutterHeight = 0;
            if (canvas)
                canvas.style.display = 'block'; // Restore canvas for next time
    ...
    function executeMenuAction() {
        ...
        if (selectedMenuIndex === 0) { // PLAY
            ...
            performImageShutterTransition(() => {
                ...
                if (daniSelectOverlay)
                    daniSelectOverlay.style.display = 'flex';
                loadDaniCourses();
            }).then(() => {
    ...
    function startDaniCourse(course) {
        ...
        daniHealth = 100;
        if (daniSelectOverlay)
            daniSelectOverlay.style.display = 'none';
        if (controlsDiv)
        ...
        try {
            ...
            if (saved) {
                ...
                if (settings.bpmNormalize !== undefined) {
                    ...
                    if (bpmNormalizeCheckbox)
                        bpmNormalizeCheckbox.checked = bpmNormalize;
                }
                if (settings.laneOpacity !== undefined) {
                    laneOpacity = parseFloat(settings.laneOpacity);
                    if (laneOpacityInput)
                        laneOpacityInput.value = (laneOpacity * 100).toString();
                    if (laneOpacityDisplay)
                        laneOpacityDisplay.textContent = (laneOpacity * 100).toString();
                }
                ...
                bpmNormalize = true;
                if (bpmNormalizeCheckbox)
                    bpmNormalizeCheckbox.checked = true;
            }
        }
        catch (e) {
    ...
    async function showResults() {
        ...
        if (customResultScreen) {
            if (valResPerfect)
                valResPerfect.textContent = stats.perfect.toString();
            if (valResGreat)
                valResGreat.textContent = stats.great.toString();
            if (valResNice)
                valResNice.textContent = stats.nice.toString();
            if (valResBad)
                valResBad.textContent = stats.bad.toString();
            if (valResMiss)
                valResMiss.textContent = stats.miss.toString();
            if (valResCombo)
                valResCombo.textContent = stats.maxCombo.toString();
            if (valResScore)
                valResScore.textContent = scaledScore.toLocaleString();
            ...
            if (resultsOverlay)
                resultsOverlay.style.display = 'none'; // Ensure old overlay is hidden
            if (canvas)
                canvas.style.display = 'none'; // Make play screen black (hidden)
    ...
    function applyModifiers(notes, assist, random) {
        return _applyModifiers(notes, assist, random, currentKeyMode);
    }
